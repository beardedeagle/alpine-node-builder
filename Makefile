.PHONY: help version test node shell build clean rebuild release
.NOTPARALLEL: build rebuild release
.DEFAULT_GOAL := help

VERSION ?= `cat VERSION`
MIN_VERSION := $(shell echo $(VERSION) | sed 's/\([0-9][0-9]*\)\.\([0-9][0-9]*\)\(\.[0-9][0-9]*\)*/\1.\2/')
IMAGE_NAME ?= beardedeagle/alpine-node-builder
...
version: ## Show Makefile version
	@echo -e 'Node version: $(shell echo $(VERSION))'
	@echo -e 'Maintainer: beardedeagle <randy@heroictek.com>'
	@echo -e 'Last modified: 2020-11-05'

test: ## Test Docker image
	@docker run --rm -it $(IMAGE_NAME):$(VERSION) node --version

node: ## Run node shell in Docker image
	@docker run --rm -it $(IMAGE_NAME):$(VERSION) node
